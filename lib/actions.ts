...
export async function updateProfile(id: string, name: string, avatarUrl?: string | null) {
    try {
        const supabase = await createClient();
        const { error } = await supabase.from('profiles').update({ name, avatar_url: avatarUrl }).eq('id', id);
        if (error) throw error;
        return { success: true };
    } catch (error: any) {
...
// Watchlist Actions
export async function addToWatchList(profileId: string, data: {
    tmdbId: number;
    mediaType: 'movie' | 'tv';
    title: string;
    releaseDate?: string;
    ...
    try {
        ...
        // Check if exists
        const { data: existing } = await supabase.from('user_media')
            .select('id')
        ...
        let error;
        if (existing) {
            const { error: err } = await supabase.from('user_media').update({
                status: data.status,
                title: data.title,
...
export async function updateProgress(profileId: string, tmdbId: number, progress: number) {
    try {
        const supabase = await createClient();
...
export async function toggleFavorite(profileId: string, tmdbId: number, isFavorite: boolean) {
    try {
        const supabase = await createClient();
        const { error } = await supabase.from('user_media')
...
export async function toggleVote(profileId: string, tmdbId: number, vote: 'like' | 'dislike' | null) {
    try {
        const supabase = await createClient();
...
export async function removeFromContinueWatching(profileId: string, tmdbId: number) {
    try {
        const supabase = await createClient();
...
export async function getWatchStatus(profileId: string, tmdbId: number) {
    try {
        const supabase = await createClient();
...
export async function getWatchList(profileId: string, status?: WatchStatus) {
    try {
        const supabase = await createClient();
        let query = supabase.from('user_media').select('*').eq('profile_id', profileId);
...
export async function getEpisodeProgress(profileId: string, tmdbId: number) {
    try {
        const supabase = await createClient();
        const { data, error } = await supabase.from('episode_progress')
            .select('*')
            .eq('profile_id', profileId)
            .eq('tmdb_id', tmdbId);
    ...
    try {
        ...
        if (existing) {
            await supabase.from('user_media').update({
                progress: progressValue !== undefined ? progressValue : (existing.progress || 0) + 1,
...
export async function updateMovieProgress(
    ...
    try {
        ...
        await incrementProgress(profileId, tmdbId, undefined, undefined, metadata ? { ...metadata, mediaType: 'movie' } : undefined, progress);
...
export async function getMovieProgress(profileId: string, tmdbId: number) {
    try {
        const supabase = await createClient();
        const { data, error } = await supabase.from('movie_progress')
            .select('*')
            .eq('profile_id', profileId)
            .eq('tmdb_id', tmdbId)
            .single();
